...
class SalesforceSinkTask extends SinkTask {
    ...
    _upsert(upserts, callback) {
        ...
        this.connection.sobject(this.properties.sObject).bulkload('upsert', {
            extIdField: this.idProperty,
            ...this.bulkOptions
        }, upserts, (error, results) => {
            if (error) {
                this._stats.bulkUpsertErrors++;
                return callback(error);
            }

            if (typeof this.bulkCallback === "function") {
                results.filter(r => r.success)
                    .map(filtered => filtered.id)
                    .then(mapped => {
                        this.connection.sobject("Account")
                            .retrieve(mapped, function (error, accounts) {
                                if (!error) {
                                    this.bulkCallback(accounts);
                                }
                            })
                    });
            }
